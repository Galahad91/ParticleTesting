Shader "Custom/Surface Lighting Custom" 
{
	Properties 
	{
		...
		_Deformation("Deformation", Range(0,0.05)) = 0.01
	}
	...
	SubShader 
	{
		...
		{
			v.vertex.xyz += v.normal * sin(_Time.w + v.vertex.x * 20.0) * _Deformation;
		...
		void surf (Input input, inout CustomSurfaceOutput output)
		{
			...
			float4 secondColor = tex2D(_SecondTex, input.uv_MainTex);
