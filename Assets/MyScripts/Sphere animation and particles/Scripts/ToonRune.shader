Shader "Custom/ToonRune" 
{
	...
	SubShader 
	{
		...
		{
			float4 mainTexCol = tex2D(_MainTex, input.uv_MainTex) * _Color;
			float4 secondaryTexCol = tex2D(_SecondaryTex, input.uv_MainTex + float2(_Time.y/2, 0)) * _Emission;

			float blendAmount = tex2D(_Mask, input.uv_MainTex).r;
